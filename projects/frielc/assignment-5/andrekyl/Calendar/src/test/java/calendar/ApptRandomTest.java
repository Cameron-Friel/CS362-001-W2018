			...
			for (int i = 0; i < NUM_TESTS; i++) {
					String methodName = ApptRandomTest.RandomSelectMethod(random);
					   if (methodName.equals("setTitle")){
						   ...
						   /*int rand = ValuesGenerator.getRandomIntBetween(random, -5, 15);
						   appt.setStartMonth(rand);
						   if (rand < 0 || rand > 12 ) {
						   		appt.setStartMonth(ValuesGenerator.getRandomIntBetween(random, 1, 12));
						   }*/
					   }
							...
							int [] recurDays = null;
							if (ValuesGenerator.getRandomIntBetween(random, 0, 5) == 3) {
								recurDays = null;
							}
							else {
								int sizeArray = ValuesGenerator.getRandomIntBetween(random, 0, 5);
